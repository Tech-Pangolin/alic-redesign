...
export function generateStaticParams() {
  return getArticlesByAudience("carriers-brokers").map((article) => ({
    slug: article.id,
  }));
}
...
export default async function CarriersResourceArticlePage({
  params,
}: PageProps) {
  const { slug } = await params;
  const article = getResourceArticle(slug);
  if (!article || article.audience !== "carriers-brokers") notFound();
