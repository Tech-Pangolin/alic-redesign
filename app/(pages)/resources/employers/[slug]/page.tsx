...
export function generateStaticParams() {
  return getArticlesByAudience("employers").map((article) => ({
    slug: article.id,
  }));
}
...
export default async function EmployerResourceArticlePage({
  params,
}: PageProps) {
  const { slug } = await params;
  const article = getResourceArticle(slug);
  if (!article || article.audience !== "employers") notFound();
